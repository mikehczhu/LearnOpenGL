...
int main()
{
	...
	lightModel = glm::translate(lightModel,glm::vec3(-0.7f, 0.5f, 1.0f));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform3f(glGetUniformLocation(ourShader.ID, "lightPos"), -0.7f, 0.5f, 1.0f);
		glBindVertexArray(VAO);
		glDrawArrays(GL_TRIANGLES, 0, 36);

		lightShader.use();
		glUniformMatrix4fv(glGetUniformLocation(ourShader.ID, "model"), 1, GL_FALSE, &lightModel[0][0]);
		glUniformMatrix4fv(glGetUniformLocation(ourShader.ID, "view"), 1, GL_FALSE, &view[0][0]);
		glUniformMatrix4fv(glGetUniformLocation(ourShader.ID, "projection"), 1, GL_FALSE, &proj[0][0]);
		glBindVertexArray(lightVAO);
		glDrawArrays(GL_TRIANGLES, 0, 36);
