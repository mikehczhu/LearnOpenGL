...
int main()
{
	...
	Shader grassShader("vertex.vs", "transparentFragment.fs");
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	data = stbi_load("F:\\vs2017\\LearnOpenGL\\Textures\\textures\\grass.png", &width, &height, &nrChannels, 0);
	if (data)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		grassShader.use();
		glUniformMatrix4fv(glGetUniformLocation(grassShader.ID, "view"), 1, GL_FALSE, &view[0][0]);
		glUniformMatrix4fv(glGetUniformLocation(grassShader.ID, "projection"), 1, GL_FALSE, &proj[0][0]);
		...
		for (GLuint i = 0; i < vegetation.size(); i++)
		{
			...
			glUniformMatrix4fv(glGetUniformLocation(grassShader.ID, "model"), 1, GL_FALSE, &model[0][0]);
			glBindVertexArray(grassVAO);
			glDrawArrays(GL_TRIANGLES, 0, 6);
		}
